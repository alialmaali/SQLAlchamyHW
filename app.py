...
def justice_league_character2(start,end=False):
    ...
    if end == False:
        ...
        df2 = session.query(func.max(Measurement.tobs), func.min(Measurement.tobs), func.avg(Measurement.tobs), Measurement.date).group_by(Measurement.date).filter(Measurement.date >= start).all()

    else:

        sql += " `date` > '"+str(start)+"' AND `date` < '"+str(end)+"'"

        df2 = session.query(func.max(Measurement.tobs), func.min(Measurement.tobs), func.avg(Measurement.tobs), Measurement.date).group_by(Measurement.date).filter(Measurement.date >= start).filter(Measurement.date <= end).all()
